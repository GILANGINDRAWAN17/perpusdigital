...
<aside class="w-64 bg-[#004d4d] text-white flex flex-col sticky top-0 h-screen shrink-0">
    ...
    <nav class="flex-1 space-y-2 mt-8">
        ...
        {{-- DASHBOARD --}}
        <a href="
            {{ $role == 'anggota' ? '/dashboard' : '' }}
            {{ $role == 'petugas' ? '/dashboardpetugas' : '' }}
            {{ $role == 'kepala_perpustakaan' ? '/dashboardkepalaperpus' : '' }}
        "
        ...
        {{-- ================= SHARED (PETUGAS + KEPALA) ================= --}}
        @if (in_array($role, ['petugas', 'kepala_perpustakaan']))
            <a href="{{ route('daftarbuku') }}"
                class="flex items-center gap-3 px-6 py-3 transition-all duration-300 text-teal-100
            {{ request()->routeIs('daftarbuku') || request()->routeIs('buku.*') ? 'bg-[#003d3d] text-white shadow-lg' : 'hover:bg-[#003d3d] hover:text-white hover:-translate-y-1 hover:shadow-lg hover:pl-5' }}">
    ...
    <div class="mb-6">
        <a href="
            {{ $role == 'anggota' ? '/profile' : '' }}
            {{ $role == 'petugas' ? '/profilepetugas' : '' }}
            {{ $role == 'kepala_perpustakaan' ? '/profilekepala' : '' }}
        "
            class="flex items-center gap-3 px-6 py-3 transition-all duration-300 text-teal-100
